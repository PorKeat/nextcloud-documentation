...
The policies are defined in [`manifests/18-cilium-nextcloud-isolation.yaml`](../manifests/18-cilium-nextcloud-isolation.yaml):
...
```bash
kubectl apply -f manifests/18-cilium-nextcloud-isolation.yaml
```
...
```bash
kubectl delete -f manifests/18-cilium-nextcloud-isolation.yaml
```
