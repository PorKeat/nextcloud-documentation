#!/bin/bash
# ==============================================================================
# Node 1 Recovery, Loopback Fix & Global Audit Setup
# Run this script on Node 1 (10.1.16.11) as root once it boots up.
# ==============================================================================
set -e

echo "==> Step 1: Fixing /etc/hosts for Multi-Master API Server Loopback..."
sed -i 's/10.1.16.11 lb.kubesphere.local/127.0.0.1 lb.kubesphere.local/g' /etc/hosts

echo "==> Step 2: Installing Global 'cluster-audit' command..."
cat << 'AUDIT_EOF' > /usr/local/bin/cluster-audit
#!/bin/bash
set -e
...
NC_READY=$(kubectl get deployment nextcloud -n nextcloud-system -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
...
DB_PRIMARY=$(kubectl get cluster nextcloud-db -n nextcloud-system -o jsonpath='{.status.currentPrimary}' 2>/dev/null || echo "")
...
SECCOMP=$(kubectl get deployment nextcloud -n nextcloud-system -o jsonpath='{.spec.template.spec.securityContext.seccompProfile.type}' 2>/dev/null || echo "")
NO_PRIV_ESC=$(kubectl get deployment nextcloud -n nextcloud-system -o jsonpath='{.spec.template.spec.containers[0].securityContext.allowPrivilegeEscalation}' 2>/dev/null || echo "")
...
chmod +x /usr/local/bin/cluster-audit
echo "==> Step 3: Running Enterprise Health Audit..."
/usr/local/bin/cluster-audit
